...
if __name__ == "__main__":
    ...
    if args.success:
        ...
        generator.update_index_success_message(args.success.replace('\\n', '\n'))
    ...
    if args.encrypt and args.vm_file and args.public_key:
        create_tinfoil_index(generator.index, Path(args.index_file), compression_flag, rsa_pub_key_path=Path(args.public_key), vm_path=Path(args.vm_file))
    else:
        create_tinfoil_index(generator.index, Path(args.index_file), compression_flag)
